...
Two rules are under test and they are separate on purpose:
  ...
  * a ride is only writable when its source is ours *and* its event was signed by a key we
    hold the nsec for — only then does a rewrite replace the ride instead of forking it
    (see hitch/blueprints/utils/ride_sources.py).
"""
...
# A key we do not hold the nsec for: the bulk imports went out under one of these, and so
# did every ride another platform published. Either way we cannot replace the event.
FOREIGN_PUBKEY = "c" * 64
    ...
    with app.app_context():
        ...
        _db.session.add_all(
            [
                ...
                # Our source, but signed by a key whose nsec we do not hold.
                _make_ride("hitchwiki.org-imported", source="hitchwiki.org", pubkey=FOREIGN_PUBKEY),
                _make_ride("liftershalte.info-imported", source="liftershalte.info", pubkey=FOREIGN_PUBKEY),
...
class TestWhichRidesAreWritable:
    ...
    @pytest.mark.parametrize(
        ...
        [
            ...
            # Our source names, but signed by a key we cannot sign a replacement with —
            # a rewrite would fork the ride rather than replace it.
            ("hitchwiki.org-imported", False),
            ("liftershalte.info-imported", False),
