...
class maps_geocode
{
	 ...
	 */
        ...
        public $format =        "json"; // json | array |debug
	...
	public function set_format($format) {
                if(!empty($format)) {
                    ...
                    if($format == "json" OR $format == "array" OR $format == "debug") {
                        $this->format = $format;
		    }
		}
	}
	...
	public function geocoder_output($data) {
		...
		if($this->format == "debug") return '<pre>'.print_r($data,true).'</pre>';
		
                elseif($this->format == "array") return $data;
