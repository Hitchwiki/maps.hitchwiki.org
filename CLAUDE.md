...
- **Run All Scripts**: `flask generate-all` - Runs the scripts needed to populate `dist/` on first boot, only if their output doesn't already exist (cron keeps them fresh afterwards), including map data, statistics aggregates, and city pages (see `hitch/__init__.py`).
...
- `sw.js` normalises `/spot/<id>` and `/dir/<from>/<to>` to `/` in its cache key: both render the same template (only the OG tags differ, and crawlers don't run the service worker), so they share one cached copy and work offline. The `/<lang>` prefix is **kept** in that key (`/mn/spot/<id>` → `/mn/`) — those pages differ in every translated string, so collapsing them onto `/` would serve an English map offline.
- **Every one of these paths is also served under `/<lang>`, and the client has to know it.** `map.js` `appPath()` strips the prefix before matching `SPOT_PATH_RE` / `COUNTRY_PATH_RE` / `DIR_PATH_RE`, and `langPath()` puts it back when writing the address bar (`routing.js` wraps both as `routePath`/`localisedPath`). `LANG_PREFIX` comes from `window.__LANG__`, set in `map.html` before `map.js` loads. Root-anchored regexes were the original bug: a shared `/mn/spot/<id>` opened the **bare world map**, because nothing recognised the path that named the spot. `langPath("/")` keeps its trailing slash (`/fi/`), the URL Flask actually serves the mirror at. Tests: `tests/map_lang_paths.test.js`, which slices the URL-scheme block out of `map.js` and evals it (the file needs a DOM, so it can't be `require`d).
- **Only the unprefixed `/spot/<id>` may be indexed.** A spot's rides are the same text in all 31 languages — only the furniture around them is translated — so the mirrors are thin duplicates, and Google had indexed a scattering of them. `LANG_MIRROR_NOINDEX_PREFIXES` (`hitch/__init__.py`) sends `X-Robots-Tag: noindex` on `/<lang>/spot/…`; `NOINDEX_PREFIXES` (`/account/`, `/dir/`, the report/accept actions) applies in **every** language. Both are matched against `strip_lang_prefix(request.path)` — writing a rule as `path.startswith("/account/")` exempts 30 of the 31 URLs a view answers on, which is exactly how `/fi/account/<name>` got indexed. A page in either set emits **no `hreflang` alternates** (`emit_hreflang`, consumed by `base.html`): an alternate pointing at a noindexed URL is a contradiction that costs the whole cluster. Its canonical stays self-referencing anyway (`inject_canonical_url`): canonicalising a mirror onto the English page is the same contradiction from the other side, and `tests/test_canonical_urls.py` pins that rule. The spot share button deliberately copies the unprefixed URL. Robots.txt must keep allowing all of it, or the noindex is never crawled and the indexed URLs never drop out.
...
### `/hitchhiking-safety` — "would you accept this ride again?", by cohort
The one safety-adjacent question the ride form asks, sliced by a cohort the visitor builds at the top of the page: up to four hitchhikers, each by gender, age at the time of the ride, and years of hitchhiking experience. Route in `main.py` (`hitchhiking_safety`), template `hitchhiking_safety.html`, logic in `hitch/static/safety_stats.js` (pure, Node-testable) + `hitch/static/hitchhiking_safety.js` (DOM), data precomputed by `hitch/scripts/hitchhiking_safety.py` into `dist/hitchhiking_safety.json` (daily, 07:30).

- **The payload is one row per answered ride, not an aggregate.** The cohort is combinatorial — four person slots × gender × age × experience — so any precomputed cube would either explode or decide in advance which questions are askable, and the page's whole point is that the visitor picks the question. Only ~1.1k of ~84k rides carry an answer, so the rows are *smaller* than a cube would be: 266 KB, 44 KB gzipped. `hitch/blueprints/utils/safety_statistics.py` builds them from raw DB rows (no app context, testable like `wait_time_statistics`).
- **The page says what was asked before it says the number.** 97% answered "yes" — read as "hitchhiking is 97% safe" that is false twice over: the question is about *one driver* on a ride that already happened, and the answers are self-selected (people who log rides, about rides that ended well enough to log). 31 "no" answers exist in the whole corpus, and the page links every one of them to its `/ride/<d>` page: a rate with no way through to the rides behind it is a claim, not evidence.
- **Every percentage carries a 95% Wilson interval**, and rows under `MIN_SAMPLE` (10) are marked. Wilson rather than the normal approximation because most cells are tiny and many are 100% "yes", where the normal interval collapses to zero width and renders "certainly safe" out of four rides.
- **Two filters must match two *different* hitchhikers** (`assignable`, exhaustive backtracking over ≤5 people × ≤4 filters). Greedy left-to-right matching gets this wrong: it hands the only 25-year-old woman to the loose filter and then fails a cohort that is satisfiable. The "exactly this many" toggle is the difference between "two women were in the car" and "two women, alone".
- **A narrowed range excludes unrecorded values by default** (`rangeMatches`), with an explicit opt-in checkbox. ~40% of answered rides have no age; a band that silently swept them in would not be that band.
- **Age is stored per ride, not as a birth year**, computed against the ride's own year (departure time first, submission time only as fallback — a summer trip logged in January must not age its rider). The *hour* has no such fallback: a submission stamp is when someone typed the ride in, and the time-of-day breakdown would be measuring that instead.
- **Rides deleted by their author or hidden by reports are excluded**, the same rule `show.py` applies to every map output. Pickup countries come from the offline `reverse_geocoder` index, in the cron script only — it costs ~30 MB resident and must not land in the waitress workers (same rule `/auto-trip` follows).
- Labels (genders, negative-experience tags, vehicles, signals, bands) are resolved in the route and injected as `window.__SAFETY_LABELS__`, never baked into the JSON: one generated file serves all 31 language mirrors.
...
- **ride_collection_statistics.py**: Weekly ride totals by source for `/statistics/ride-collection` (daily)
...
- **hitchhiking_safety.py**: Per-ride rows behind `/hitchhiking-safety` (daily, 7:30 AM) — the rides carrying a "would you accept this ride again?" answer, one compact row each, filtered in the browser. See the section of the same name above for why it is not an aggregate and what the page has to say before it shows a number. Reads the ride table directly (no `dist/` dependency) and reverse-geocodes each pickup into a country with the offline index, so it stays out of the request path. Cheap (~15 s), nearly all of it the geocoder's one-off index build.
...
- **build_ride_routes.py**: Builds the routing graph from rides (daily at 2 AM). For every ride with a destination it fetches an OSRM driving route and records which *other* known start spots lie within 300 m of the polyline, in travel order; sequences shared by ≥2 rides become the "repeatable" trees in `dist/repeatable_routes.json` (read by `static/routing.js`). The same run also writes `dist/oneoff_routes.json` — the identical trees with the ≥2 threshold dropped to 1 (so a corridor a single ride established still forms edges), indexing the *same* `spots` array and carrying a `spot_count` guard. The routers fetch/build it lazily and search it **only when the repeatable graph connects nothing** (`routing.js` `loadFallbackRouter` / `repeatable_router.py` `load_oneoff_router`); it's a ~6× larger superset (~3.4 MB), so it must not load on every map view. Routes that use a support-1 leg are flagged in the UI ("1 logged ride"). Standalone script (plain `python3`, not `flask generate`), reads SQLite directly. OSRM responses are cached in `dist/route_cache.jsonl` (read via a byte-offset index so geometries never all sit in RAM), so a daily run only fetches routes for rides added since the last run. **The cache is bounded at `ROUTE_CACHE_MAX_BYTES` (100 MB).** It had grown unbounded to 1.2 GB — the largest single file on a disk that has hit 100% — and two separate mechanisms bring it down, in this order of importance: geometry is stored as an **encoded polyline** rather than a JSON float array (1202 MB → 170 MB, ~86%, no re-fetching and no change to what the join finds), and only then does the byte cap apply, enforced by `compact_route_cache` after each run. The encoding is what does the real work: at the old ~36 KB/route a 100 MB cap would hold only ~2.8k of 33k routes, so ~92% would be re-fetched nightly from OSRM's **public demo server** — encoded, the same cap holds ~19.5k. `ROUTE_CACHE_PRECISION` is 5 (~0.6 m, vertex count preserved exactly); measured on 400 real routes against the live 38k spots, 397 gave an identical passed-spot set and the 3 differing spots out of 5192 hits all sat at 299.7–299.9 m against the 300 m threshold. **Compaction only runs on a full pass** — a `--limit` run touches an arbitrary subset, so "not used this run" would wrongly mark thousands of live routes stale. `--recompact-cache` re-encodes an existing cache in place (used once to migrate the legacy 1.2 GB file, with a generous `--cache-max-bytes` so nothing was evicted); reads accept both the compact `geom` polyline and the legacy `geometry` array. Spot consolidation must stay identical to `show.py`'s (5 m merge → service-area/road-island polygon grouping → snap onto `dist/spots.json`), otherwise routes reference phantom spots with no map marker.
