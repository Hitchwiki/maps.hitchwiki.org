...
- **SupersededRideEvent**: `(pubkey, d)` coordinates this app has rewritten under its current Nostr key. Which rides a user may edit/claim is decided by `hitch/blueprints/utils/ride_sources.py` — the ride's `source` must be one of ours (everything but triphopping.com today) **and** its event must be signed by one of our keys, which includes the older key the bulk imports went out under. Rewriting one of those imported rides cannot replace the original (kind 36820 is addressable per `(pubkey, kind, d)`, and a NIP-09 delete would have to be signed by that old key), so the new event lands *beside* it on the relays. This table is how the old copy is kept off the map: `_retire_superseded_event` (`main.py`) records the coordinate and drops the stale row, and **both fetch scripts skip anything listed here** — without that, the weekly full `fetch_nostr` re-imports the pre-edit copy and the ride shows twice. Never truncated, unlike `ride_event`, which is exactly why it is a separate table
...
```
`hitch/scripts/migrate_superseded_ride_events.py` is the same pattern for `superseded_ride_event` (applied to `hitchhiking-prod.sqlite` on 2026-07-31). Note this one is needed by the **cron fetch scripts**, not just a route: without the table, every `fetch_nostr` / `fetch_nostr_incremental` run crashes on `no such table` and rides silently stop arriving.
