...
### MCP server (`/mcp`, `hitch/blueprints/mcp.py`)
Lets AI assistants plan real hitchhiking routes from our data instead of paraphrasing whatever they once crawled. This is the *answer* to `/dir/<from>/<to>` being permanently `noindex` (quadratic URL space, see the URL scheme section): a crawler can't enumerate our routes, but an assistant can ask for one.

- **Exactly two tools, `search` and `fetch`** — the pattern OpenAI requires. Outside developer mode ChatGPT **rejects** an MCP server that lacks them, and Deep Research calls only these two regardless of what else is offered. Both take a single string, which is what lets a client drive the server knowing nothing about hitchhiking. Adding a third tool is not free: it's dead weight for the clients that matter.
- **Ids must round-trip statelessly.** Whatever `search` emits comes back to `fetch` later, possibly on another connection; there is no session. So an id *contains* its document — `spot:<lat>_<lon>`, `route:<lat>,<lon>__<lat>,<lon>` — rather than indexing into server-side state. `_parse_document_id` also accepts the bare `<lat>_<lon>` form and the full `maps.hitchwiki.org` URL, because models routinely pass back the `url` field instead of the `id`.
- **Every result needs a non-empty `url` or it isn't citable** — that is the entire point of answering: the assistant renders a link chip back to the site. Likewise the *result payload must be emitted twice*, as `structuredContent` **and** as a JSON-encoded string in `content[0].text`; emitting one and not the other is the usual reason a technically-correct server reads as empty.
- **The search/fetch split lands on our cheap/expensive boundary**, which is lucky rather than designed. `search` only geocodes and scans `spots.json` (~0.3 s) and **must never route**; the ~190 MB, ~3 s routing graph is forked only in `fetch`, and only if the model actually opens that result. There's a test asserting `search` doesn't call the router.
- **Intent parsing is the one lossy step.** `search` gets free text and must decide "route from X to Y" vs "spots around here". The trap: *"how to hitchhike out of Berlin" contains `" to "`* and would split into `("how", "hitchhike out of berlin")`, so question scaffolding is stripped before any separator is sought. Second guard: both halves must geocode, otherwise the split found a stray "to" — and if only one half resolves we answer about *that* half rather than geocoding the whole string, which resolves to nothing.
- **Route document titles use `_reverse_label`** (a live Photon reverse geocode), not the offline `_spot_name`: the title is what an assistant reads out and cites, and a route's endpoints are usually city centres where no spot sits, so `_spot_name` would yield bare coordinates. Only two per document, so the round trips are affordable inside a fetch that already forks the router. Leg names still use the offline `_spot_name` — a 7-leg route would otherwise be seven extra round trips. **No `reverse_geocoder` fallback** (unlike `route_preview.py`): ~30 MB in a long-lived waitress worker on an OOM-prone host.
- **Transport is stateless Streamable HTTP.** One JSON-RPC message per POST, one `application/json` reply — the spec allows a plain JSON response wherever the server has nothing to stream. That means **no new dependency and no asyncio**: the official Python MCP SDK is ASGI/Starlette and this app is WSGI under waitress. Consequences, both spec-legal: no `Mcp-Session-Id`, and `GET /mcp` returns 405. Notifications (no `id`) get `202` with an empty body — replying to one is a protocol violation strict clients reject.
- **Guards, because this endpoint is public, unauthenticated and cheap to call in a loop** (unlike `/dir/`): a `BoundedSemaphore` caps concurrent routing forks at 2 (waitress is one process with a thread pool, so a module-level semaphore covers it), and the alternatives/walk-distance are server-side constants since there are no typed arguments to carry them.
- **`route_query.py` imports its engine by path, not as `hitch.scripts.repeatable_router`** — importing the `hitch` package executes `hitch/__init__.py` and pulls the entire Flask app into a process whose whole point is to stay small. Standalone `python3` script like `build_ride_routes.py`. (`route_preview.py` predates this and still takes the package import.)
- Results cache to `dist/mcp/<key>.json`, invalidated by comparing mtime against `dist/repeatable_routes.json` so a cached itinerary is never served after the nightly `build_ride_routes.py` learns new corridors. Cold fetch ~6 s (routing + two reverse geocodes), warm ~0 s. Pruned daily (see Cron Schedule).
- `GET /.well-known/mcp.json` is a machine-readable pointer for clients/directories that probe a domain. Tests: `tests/test_mcp_server.py`.
- **Not mirrored per-language** (unlike `main_bp`/`user_bp`): it renders no localised text, so `/de/mcp` would be a second URL for a byte-identical API.
- **Reaching users is a separate problem from building this.** Connectors are opt-in: someone pastes the URL into ChatGPT developer mode, or picks it in Deep Research. Only an Apps-SDK directory submission makes it available without setup, and *no* MCP path makes an assistant reach for us unprompted — that remains a job for indexable pages.
...
- **Daily at 1:35 AM**: prune `dist/mcp/` MCP route cache — same `find -mtime +7 -delete`, same reasoning (unbounded key space)
